...
1. **Review** — a list of the detected apps, all pre-selected (shown in **green**; the cursor row is marked with a green `▌` bar). `↑`/`↓` move, `Backspace` toggles a row between selected (green) and deselected (dim), `Enter` confirms. Deselect anything you don't want (e.g. the terminal, file-explorer windows).
